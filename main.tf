locals {
  ...
  launch_template_name = coalesce(var.fullname, format("%s-%s-lt", local.name_prefix, var.name))
  security_group_ids = length(var.network_interfaces)>0||length(var.security_group_ids)<1 ? [] : var.security_group_ids
}
...
resource "aws_launch_template" "this" {
  ...
  dynamic "network_interfaces" {
    for_each = length(var.network_interfaces) > 0 ? [true] : []
    content {
  ...
  dynamic "placement" {
    for_each = length(var.placement) > 0 ? [true] : []
  ...
  dynamic "tag_specifications" {
    ...
    content {
      ...
      tags = merge(local.tags, { Name = local.launch_template_name }, local.tags)
    }
  }
